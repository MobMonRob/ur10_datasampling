#include "ros/ros.h"
#include "ur10_control/GoToPosition.h"
#include <std_msgs/Float64.h>

// Global joint publisher variables
ros::Publisher joint1_pub, joint2_pub, joint3_pub, joint4_pub;

// This function checks and clamps the joint angles to a safe zone
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2, float requested_j3 ,float requested_j4)
{
    ...
    float min_j1, max_j1, min_j2, max_j2, min_j3, max_j3, min_j4, max_j4;
    ...
    std::vector<float> clamped_data = { clamped_j1, clamped_j2, clamped_j3, clamped_j4 };

    return clamped_data;
}

// This callback function executes whenever a safe_move service is requested
bool handle_safe_move_request(ur10_control::GoToPosition::Request& req,
    ur10_control::GoToPosition::Response& res)

{

    ROS_INFO("GoToPositionRequest received - j1:%1.2f, j2:%1.2f", (float)req.joint_1, (float)req.joint_2, (float)req.joint_3, (float)req.joint_4);

    // Check if requested joint angles are in the safe zone, otherwise clamp them
    std::vector<float> joints_angles = clamp_at_boundaries(req.joint_1, req.joint_2, req.joint_3, req.joint_4);
    ...
    res.msg_feedback = "Joint angles set - j1: " + std::to_string(joints_angles[0]) + " ,j2: " + std::to_string(joints_angles[1]) + " ,j3: " + std::to_string(joints_angles[2]) + " ,j4: " + std::to_string(joints_angles[3]);
...
int main(int argc, char** argv)
{
    ...
    joint1_pub = n.advertise<std_msgs::Float64>("/ur10/joint_1_position_controller/command", 10);
    joint2_pub = n.advertise<std_msgs::Float64>("/ur10/joint_2_position_controller/command", 10);
    joint3_pub = n.advertise<std_msgs::Float64>("/ur10/joint_3_position_controller/command", 10);
    joint4_pub = n.advertise<std_msgs::Float64>("/ur10/joint_4_position_controller/command", 10);
